...
func TestTransactionsWithSortByAmountDesc(t *testing.T) {
	userID := int64(1)
	...
	transactions.Value("transactions").Array().First().Object().ValueEqual("amount", max)
	transactions.Value("transactions").Array().Last().Object().ValueEqual("amount", min)
